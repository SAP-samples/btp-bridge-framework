...
class IntegrationSuite {
  ...
  searchObject(paramPathArray, query, interfaceMapping, commonConfig) {
    return new Promise ( (resolve, reject)=>{
      ...
      const iFlowKeys = this.constructIFlowKeysCollection(interfaceMapping, paramPathArray);
      const jsonBody = {
      ...
      if(objectMapping.searchFields){
        ...
        objectMapping.searchFields.forEach((field) => {
          ...
          if (query["filterValue"]) {
            filters.push(
              `substringof(` + field + `,'` + query["filterValue"] + `') eq true`
            );
          } else {
  ...
  getObject(paramPathArray, interfaceMapping, commonConfig, btpAccessToken) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let objectMapping = interfaceMapping[businessObject];
      const iFlowKeys = this.constructIFlowKeysSingleObject(interfaceMapping, paramPathArray);
      const jsonBody = {};
  ...
  updateObject(paramPathArray, interfaceMapping, commonConfig, response, body) {
    return new Promise((resolve, reject) => {
      let businessObject = paramPathArray[paramPathArray.length-2];
      let objectMapping = interfaceMapping[businessObject];
      const iFlowKeys = this.constructIFlowKeysSingleObject(interfaceMapping, paramPathArray);
